...
############################ VARIABLES NATIVAS ##################################
table <- data.frame(gs_read(sheet,ws = "Variables nativas"))
table <- data.frame(table[1:259,1:11])

# Dividimos las variables en tres tipos:
#Tabla Cuantitativas
table_cuan <- subset(table,table$Tipo == 'Cuantitativo')
table_cuan <- subset(table_cuan,table_cuan$Uso == 'Sí')
#Tabla Cualitativas
table_cual <- subset(table,table$Tipo == 'Cualitativo')
table_cual <- subset(table_cual,table_cual$Uso == 'Sí')
table_cual <- subset(table_cual,table_cual$Formato == 'CADENA')
#Tabla Booleana
table_bool <- subset(table,table$Tipo == 'Booleana')
table_bool <- subset(table_bool,table_bool$Uso == 'Sí')
table_sin_uso <- subset(table,table$Tipo == '--')
#Tipo de variable, si CD o var_nativa
tipo <- "var_nativa"
...
for(j in 1:NROW(cadenas_cual)){
  ...
  system.time(
    output_query <- foreach(i = 1:NROW(table_cual), .packages = "bigrquery")%dopar%{
      variable_beauty <- paste("Variable",table_cual[i,4],sep = " ")
      variable <- (table_cual[i,4])
...
for (i in 1:NROW(table_cual)){
  ...
  output <- paste0("htmls/Categoricas/variable_",table_cual[i,4],".html")
...
for(j in 1:NROW(cadenas_cuan)){
  ...
  system.time(
    output_query <- foreach(i = 1:NROW(table_cuan), .packages = "bigrquery")%dopar%{
      variable_beauty <- paste("Variable",table_cuan[i,4],sep = " ")
      variable <- (table_cuan[i,4])
...
for (i in 1:NROW(table_cuan)){
  ...
  ################### Borrar #################
  # tabla_evol_qago <- query_evol_qago(variable,tipo)
  # data_evol_ago <- data.frame(tabla_evol_qago)
  # data_evol_ago <- data_evol_ago %>% mutate(cd_avg = mean(cd))
  # tabla_evol_qsep <- query_evol_qsep(variable,tipo)
  # data_evol_sep <- data.frame(tabla_evol_qsep)
  # data_evol_sep <- data_evol_sep %>% mutate(cd_avg = mean(cd))
  # tabla_evol_qoct <- query_evol_qoct(variable,tipo)
  # data_evol_oct <- data.frame(tabla_evol_qoct)
  # data_evol_oct <- data_evol_oct %>% mutate(cd_avg = mean(cd))
  # tabla_evol_qnov <- query_evol_qnov(variable,tipo)
  # data_evol_nov <- data.frame(tabla_evol_qnov)
  # data_evol_nov <- data_evol_nov %>% mutate(cd_avg = mean(cd))
  # tabla_evol_qdic <- query_evol_qdic(variable,tipo)
  # data_evol_dic <- data.frame(tabla_evol_qdic)
  # data_evol_dic <- data_evol_dic %>% mutate(cd_avg = mean(cd))
  # tabla_evol_qene <- query_evol_qene(variable,tipo)
  # data_evol_ene <- data.frame(tabla_evol_qene)
  # data_evol_ene <- data_evol_ene %>% mutate(cd_avg = mean(cd))
  ##############################
  ...
  output <- paste0("htmls/Cuantitativas/variable_",table_cuan[i,4],".html")
  rmarkdown::render(input = input, output_file = output)
}
...
for(j in 1:NROW(cadenas_bool)){
  ...
  system.time(
    output_query <- foreach(i = 1:NROW(table_bool), .packages = "bigrquery")%dopar%{
      variable_beauty <- paste("Variable",table_bool[i,4],sep = " ")
      variable <- (table_bool[i,4])
...
for (i in 1:NROW(table_bool)){
  ...
  output <- paste0("htmls/Booleanas/variable_",table_bool[i,4],".html")
  rmarkdown::render(input = input, output_file = output)
}
